        ...
        <div v-for="order in orders" :key="order.orderId" v-show="getShipGroups(order.items).length > 0">
          <ion-card v-for="(shipGroup, index) in getShipGroups(order.items)" :key="index" @click.prevent="viewOrder(order, shipGroup)">
            <ion-item lines="none">
              ...
              <div class="metadata">
                <ion-badge v-if="order.date" color="dark">{{ moment.utc(order.date).fromNow() }}</ion-badge>
    ...
    async getPickupOrders (vSize?: any, vIndex?: any) {
      ...
      const payload = {
        "inputFields":{
          ...
          "facilityId_fld0_value": this.currentFacility.facilityId,
          "facilityId_fld0_op": "equals",
          "facilityId_fld0_grp": "1",
          "shipFromFacilityId_fld1_value": this.currentFacility.facilityId,
          "shipFromFacilityId_fld1_op": "equals",
          "shipFromFacilityId_fld1_grp": "2"
        },
        viewSize,
        viewIndex,
        "fieldList": [ "orderId" ],
